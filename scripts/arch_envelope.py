#!/usr/bin/env python3
"""Gate 3: architecture-reviewer output -> shared ReviewEnvelope.

Converts one human-graded arch review into a schema-valid assurance
fragment: claim C2 (review matches its frozen expected verdict)
mirrors the human grade, which is the measurement instrument here
(no mechanical key exists for architecture judgment; expected.json
is the frozen answer key). A single reviewer can never ACCEPT alone.

Usage:
    python3 scripts/arch_envelope.py --case a --review <file>
        --grade '{"verdict": "conforms"}' --run-id <id> --out <case.json>
"""
import argparse
import json
import sys
...
STATUS = {"conforms": "supported", "violates": "defeated",
...
def build_case(case, review_path, grade, run_id, out_path=None,
    ...
    casedir = REPO / "tests" / "arch-cases" / ("arch-%s" % case)
    ...
    if verdict not in STATUS:
        raise ValueError("unknown arch verdict: %r" % verdict)
    status = STATUS[verdict]
    context_id = "arch-case-%s" % case
    base_rev = "fixture:arch-%s:repo@%s" % (case, shared.dir_hash(
        casedir / "repo")) if (casedir / "repo").is_dir() \
        else "fixture:arch-%s:no-repo" % case
    cand_rev = "fixture:arch-%s:candidate@%s" % (case, shared.dir_hash(
        casedir / "candidate")) if (casedir / "candidate").is_dir() \
        else "fixture:arch-%s:no-candidate" % case
    ...
    if suite_dir.is_dir():
        ...
        suite_ev = shared.ev(
            "E2-%s-suite" % case, "suite: %s (%s)" % (first, ran_note),
        ...
        suite_ev = shared.ev(
            "E2-%s-suite" % case, "no suite declared (no executable claim)",
    ...
    ev_git = shared.ev(
        "E2-%s-git" % case, "repo HEAD observed (%s)" % git["status"],
    ...
    claim = {
        "id": claim_id,
        "statement": "Candidate arch-%s conforms to its documented architecture (per human-graded review)" % case,
    ...
    skill_sha = shared.sha((REPO / "skills" / "architecture-reviewer" /
                            "SKILL.md").read_bytes())
    uncertainty = {
        "id": "U2-%s" % case,
    ...
    acase = shared.assemble(
        module="architecture-reviewer", short="arch", case_tag=case,
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="arch envelope adapter")
    ap.add_argument("--case", required=True)
    ap.add_argument("--review", required=True)
    ap.add_argument("--grade", required=True,
                    help='human grade JSON, e.g. {"verdict": "conforms"}')
    ...
    ap.add_argument("--criteria-first", action="store_true",
                    help="caller asserts rubric recorded before candidate examined")
