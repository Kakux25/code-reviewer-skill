"""Gate 7: sociotechnical-reviewer fixtures + envelope adapter.

Records-only fixtures (ownership maps, handoff logs, team rosters):
no suites by design. Claim C6 is about the candidate (P1 lesson).
Human grading additionally enforces zero unsupported human claims.
"""
import json
import sys
import tempfile
...
class FixtureIntegrity(unittest.TestCase):
    ...
    def test_kinds_cover_gate_criteria(self):
        kinds = {json.loads((CASES / ("soc-%s" % i) / "expected.json")
...
class AdapterContract(unittest.TestCase):
    def _build(self, case, verdict):
        import socio_envelope
        from validate_assurance import validate
        tmp = tempfile.NamedTemporaryFile(
            mode="w", suffix=".txt", prefix="review-soc-%s-" % case,
        ...
        acase = socio_envelope.build_case(case, str(review), grade,
                                          run_id="unit")
        validate(acase)  # raises unless schema-valid
        return acase

    def test_coordinated_maps_to_supported(self):
        acase = self._build("c", "coordinated")
        claim = acase["claims"][0]
        self.assertEqual(claim["id"], "C6-c")
        self.assertEqual(claim["status"], "supported")
        self.assertEqual(len(claim["supporting_evidence"]), 1)
        self.assertEqual(claim["counterevidence"], [])
        self.assertIn("Candidate soc-c", claim["statement"])

    def test_uncoordinated_maps_to_defeated(self):
        acase = self._build("a", "uncoordinated")
        claim = acase["claims"][0]
    ...
    def test_unknown_maps_to_unresolved(self):
        acase = self._build("d", "unknown")
        claim = acase["claims"][0]
...
class FrozenRun(unittest.TestCase):
    ...
    def test_frozen_envelopes_validate_and_mirror(self):
        ...
        grades = json.loads((self.RUN / "grades.json").read_text(
            encoding="utf-8"))["grades"]
        want = {"coordinated": "supported", "uncoordinated": "defeated",
                "unknown": "unresolved"}
        for i in IDS:
